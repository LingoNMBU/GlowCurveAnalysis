...
adaGlow1 = AdaGlowFit(exp_data.copy(deep=True), beta=5, metric='rmse', g_max=100, N_pop=30, n_peaks=2)

adaGlow1.algorythm()

mod_data1 = adaGlow1.mod_data

plt.plot(exp_data['Temperature'], exp_data['Intensity'], label='experimental')
plt.plot(mod_data1['Temperature'], mod_data1['Intensity'], label='model peak 1')
plt.legend()
plt.show()
